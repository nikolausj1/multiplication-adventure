...
struct ParentAreaView: View {
    ...
    private var explainRows: some View {
            explainRow("star.fill",
                       "Every day is one QUEST: the app picks the facts for today's star and drills each one up its ladder (2 multiple-choice + 3 typed), mixed with review of everything learned so far. The quest is sized to about 8 minutes of answering at your child's own pace (roughly 30–50 questions) and ends when the star slams in — the bar fills one notch per answer.")
            explainRow("map.fill",
